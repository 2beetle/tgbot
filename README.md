...
> ⚠️ **配置方式**：全部变量需通过系统环境变量设置（例如 `export KEY=value`、`systemd`/`Supervisor` 的 `Environment=` 配置、Docker 的 `environment` 字段等）。
...
| `CRYPTO_PASSWORD` | 加密密码（用于敏感数据加密，建议16位以上） | `your_strong_password_123` | ✅ |
...
| 配置类型 | 配置方式 | 作用域 | 适用场景 |
|:---------|:---------|:-------|:---------|
| **环境变量** | 系统环境变量 | 全局 | 系统级配置，如 API 密钥、数据库连接 |
